...
- **功能**:
  - 请求拦截器：添加认证token
  - 响应拦截器：统一错误处理
  - 基础URL配置: http://localhost:5001/api/v1
...
4. **权限管理**: 基于角色的访问控制
5. **模块化架构**: 清晰的代码组织结构
6. **TypeScript支持**: 类型安全和更好的开发体验
7. **Docker支持**: 容器化部署方案


修复了 mapInteractions.js 中的 handleGoBack 函数，确保它接收正确的参数：

mapInstance: 地图实例
currentLevel: 当前层级 ('country', 'province', 或 'city')
currentCode: 当前区域代码
initMap: 初始化地图函数
updateMap: 更新地图函数
更新了 handleGoBack 函数的实现逻辑：

从市级返回省级时，正确提取省级代码（前两位数字+0000）
从省级返回全国时，加载全国地图数据
使用正确的参数调用 loadMapData 和 updateMap 函数
确保 EnhancedChinaMap.vue 中的 goBack 函数正确调用 handleGoBack 函数