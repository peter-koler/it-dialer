...
const columns = [
  ...
  { title: '平均连接时间', dataIndex: 'avgResponseTime', key: 'avgResponseTime',
...
const probeColumns = [
  ...
  { title: '连接时间', dataIndex: 'response_time', key: 'response_time',
    customRender: ({ text }) => text ? `${text} ms` : '-' },
  { title: '连接状态', dataIndex: 'connection_status', key: 'connection_status',
    customRender: ({ text }) => {
      const statusMap = {
        'connected': '已连接',
        'refused': '连接被拒绝',
